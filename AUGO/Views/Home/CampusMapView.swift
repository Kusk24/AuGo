import SwiftUI
import MapKit

struct CampusMapView: View {
    @Binding var showAnnouncement: Bool
    
    @EnvironmentObject var viewModel: CampusMapViewModel
    @EnvironmentObject var postManager: PostManager
    @EnvironmentObject var announcementCenter: AnnouncementCenter

    @State private var cameraPosition: MapCameraPosition = .automatic
    @State private var selectedCategories: Set<Post.Category> = Set(Post.Category.allCases)
    
    @State private var showClusterSheet = false
    @State private var selectedCluster: PostCluster?
    
    @State private var isPresentingCreatePost = false
    
    @State private var selectedAnnouncement: Announcement?
    @State private var showSingleAnnouncement = false

    var body: some View {
        ZStack {
            Color.Brand.primary.opacity(0.06).ignoresSafeArea()

            Map(position: $cameraPosition) {
                UserAnnotation()
                
                // Clusters update dynamically via the updateMapData call
                ForEach(viewModel.clusters) { cluster in
                    Annotation("", coordinate: cluster.coordinate) {
                        if cluster.count == 1 {
                            PostAnnotationView(
                                post: cluster.posts[0],
                                isSelected: viewModel.selectedPostID == cluster.posts[0].id
                            ) {
                                viewModel.toggleSelected(cluster.posts[0])
                            }
                        } else {
                            PostClusterView(cluster: cluster) {
                                selectedCluster = cluster
                                showClusterSheet = true
                            }
                        }
                    }
                }

                ForEach(announcementCenter.announcements) { ann in
                    if let coord = ann.coordinate {
                        Annotation("", coordinate: coord) {
                            AnnouncementPinView(announcement: ann) {
                                announcementCenter.markAsRead(ann)
                                selectedAnnouncement = ann
                                showSingleAnnouncement = true
                            }
                        }
                    }
                }
            }
            .mapControls {
                MapUserLocationButton()
                MapCompass()
                MapPitchToggle() // <--- THIS ADDS THE 2D/3D ICON
            }
            .clipShape(RoundedRectangle(cornerRadius: 24))
            .padding()

            VStack {
                HStack {
                    filterMenu
                    Spacer()
                }
                Spacer()
                createPostButton
            }
            .padding(30)
        }
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Button {
                    announcementCenter.markAllAsRead()
                    showAnnouncement = true
                } label: {
                    Image(systemName:
                        announcementCenter.unreadCount > 0
                        ? "bell.badge.fill"
                        : "bell.fill"
                    )
                    .symbolRenderingMode(
                        announcementCenter.unreadCount > 0
                        ? .palette
                        : .monochrome
                    )
                    .foregroundStyle(
                        announcementCenter.unreadCount > 0
                        ? AnyShapeStyle(Color.red)           // dot
                        : AnyShapeStyle(Color.Brand.primary),// bell (read)
                        Color.Brand.primary                  // bell (unread)
                    )
                }
            }
        }
        .navigationDestination(isPresented: $isPresentingCreatePost) {
            CreatePostView(isPresentedFromHome: $isPresentingCreatePost)
        }
        .sheet(isPresented: $showSingleAnnouncement) {
            if let ann = selectedAnnouncement {
                SingleAnnouncementView(announcement: ann)
            }
        }
        .sheet(isPresented: $showClusterSheet) {
            if let cluster = selectedCluster {
                ClusterPostListView(posts: cluster.posts)
            }
        }
        .onAppear {
            postManager.fetchAllPosts()
            cameraPosition = .region(viewModel.campusRegion)
            updateMapData()
        }
        .onChange(of: postManager.allPosts) {
            updateMapData()
        }
        .onChange(of: selectedCategories) {
            updateMapData()
        }
    }

    /// Filters raw posts based on selected categories and tells ViewModel to rebuild clusters
    private func updateMapData() {
        let filtered = postManager.allPosts.filter { selectedCategories.contains($0.category) }
        viewModel.updateClusters(with: filtered)
    }

    private var filterMenu: some View {
        Menu {
            Section {
                Button("Select All") {
                    selectedCategories = Set(Post.Category.allCases)
                }
                Button("Deselect All") {
                    selectedCategories = []
                }
            }
            
            Section {
                ForEach(Post.Category.allCases) { category in
                    Button {
                        if selectedCategories.contains(category) {
                            selectedCategories.remove(category)
                        } else {
                            selectedCategories.insert(category)
                        }
                    } label: {
                        Label(category.rawValue, systemImage: selectedCategories.contains(category) ? "checkmark.square.fill" : "square")
                    }
                }
            }
        } label: {
            Image(systemName: "slider.horizontal.3")
                .padding(10)
                .background(.white)
                .clipShape(Circle())
                .shadow(radius: 4)
        }
    }

    private var createPostButton: some View {
        HStack {
            Spacer()
            Button {
                isPresentingCreatePost = true
            } label: {
                Circle()
                    .fill(Color.Brand.primary)
                    .frame(width: 56, height: 56)
                    .overlay(
                        Image(systemName: "plus")
                            .foregroundColor(.white)
                            .font(.title3.bold())
                    )
                    .shadow(radius: 6)
            }
        }
    }
}
